...
基于 LazyVim + lazy.nvim 的个人配置，**仅面向 macOS 机器，且必须使用本仓库的 `macos` 分支**。
本文的依赖安装、迁移和更新步骤均以该环境为准，不适用于其他操作系统或分支。
...
代码块中的 `sh` 命令在 macOS 终端执行；以 `:` 开头的命令在 Neovim 中按 `Esc` 后输入并回车。
...
默认 macOS 机器已安装 Homebrew，且终端中可以正常使用 `brew` 命令。

### 1. 检查 Command Line Tools

先安装 Apple Command Line Tools，提供编译 Treesitter parser 等组件所需的 C 编译器：

```sh
xcode-select --install
```

按系统弹窗完成安装后再继续；已安装时可跳过，用 `xcode-select -p` 检查安装路径。

### 2. 安装基础依赖

```sh
brew install neovim git ripgrep fd tree-sitter-cli node python go tmux
```
...
| C 编译器、`tree-sitter` CLI | 编译和安装语法解析器；编译器由 Command Line Tools 提供 |
...
这里按本分支已启用的功能安装 Node.js、Python 和 Go，减少首次安装语言工具时缺少运行时的问题。
Homebrew 安装系统依赖；lazy.nvim 安装 Neovim 插件；Mason 管理语言服务器、格式化器和调试器。
这些依赖不会随配置仓库一起下载。
...
使用支持真彩色的终端，并安装 Nerd Font 3+，以正确显示启动页、文件和状态栏图标。
例如安装 [JetBrains Mono Nerd Font](https://formulae.brew.sh/cask/font-jetbrains-mono-nerd-font)：

```sh
brew install --cask font-jetbrains-mono-nerd-font
```

安装后在终端的字体设置中选择 `JetBrainsMono Nerd Font`，重新打开终端。
若要使用 `Alt+j/k`，将终端的 Option 键设置为 Alt/Meta；macOS 功能键可能需要配合 Fn。
...
```sh
brew install lazygit       # Git 交互界面
brew install imagemagick   # 图片格式转换
brew install tectonic typst  # LaTeX 公式、Typst 渲染
brew install cmake        # 使用 CMake 的 C/C++ 项目
```

已有 `pdflatex` 时可作为 LaTeX 渲染工具，无需为同一用途重复安装 Tectonic。
Snacks 图片显示依赖终端对 Kitty Graphics Protocol 的支持，安装 ImageMagick 后仍需兼容的终端。
在 tmux 中显示异常时检查 `allow-passthrough`，并运行 `:checkhealth snacks`。
终端支持和渲染依赖详见 [Snacks Image 文档](https://github.com/folke/snacks.nvim/blob/main/docs/image.md)。
...
以下使用 macOS 上 Neovim 的默认路径；若设置了 `XDG_*` 或 `NVIM_APPNAME`，请调整路径。
已有配置会被重命名备份，不会删除。
...
mkdir -p "$HOME/.config"
git clone --branch macos --single-branch https://github.com/Qizi706/nvim.git "$HOME/.config/nvim"
```
...
| TypeScript / JavaScript、Astro | `:MasonInstall vtsls astro-language-server` |
...
剪贴板使用 Neovim 默认 provider 检测，macOS 本地可检查 `pbcopy` / `pbpaste`；
`options.lua` 中 OSC 52 和 tmux 剪贴板方案均为注释状态，通过 SSH 使用时需单独检查复制粘贴。
...
| 图片与公式 | Snacks Image；文档图片使用浮窗，最大 80 列 × 30 行，关闭行内图片，启用公式渲染 |
...
| TypeScript / JavaScript / Astro | TypeScript、VTSLS、Astro 扩展 |
...
- 代码中有每 750ms 检查内容的自动刷新函数，但调用被注释，**当前不会自动刷新**。
...
| 现象 | 排查方式 |
| --- | --- |
| 提示 `nvim` 找不到 | 确认已执行基础依赖安装，并检查 Homebrew 的 PATH 设置 |
...
| Treesitter parser 安装失败 | 检查 Command Line Tools、C 编译器、Tree-sitter CLI 及下载日志 |
| Markdown 预览失败 | 在 `:Lazy` 选中 `markdown-preview.nvim` 按 `gb` 构建后重试 |
| 图片或公式不显示 | 运行 `:checkhealth snacks`；检查兼容终端、ImageMagick、公式渲染工具及 tmux 设置 |
...
若分支已分叉，`--ff-only` 会停止：先检查 `git log --oneline --left-right HEAD...origin/macos`，再决定如何合并。
...
lua/config/options.lua        全局选项、Picker 和 Python LSP 选择
